...
set.seed ( 1 )
#use only 1000 sample to save time
...
###support vector classifier, takes about 2 minutes
tune1 <- tune ( svm , f1 , 
...
#R2
tse <- sum((test_d$SWC_F_MDS_1-mean(test_d$SWC_F_MDS_1,na.rm=T))^2,na.rm=T)
sse <- sum((test_d$SWC_F_MDS_1-pred1)^2,na.rm=T)
1-sse/tse
#0.294

#support vector machine with radial kenerl function, takes about several minutes
tune2 <- tune ( svm , f1 , 
...
#R2
sse <- sum((test_d$SWC_F_MDS_1-pred2)^2,na.rm=T)
1-sse/tse
#0.245
